...
export const updateStringPropertyAsync = createAsyncThunk(
  ...
  async ({ property, value }: { property: keyof ITaleCreate; value: any }) => {
    ...
    if (currentTaleString) {
      ...
      console.error('#1 No tale found in AsyncStorage. Using default empty tale.');
...
const taleSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addCase(removeTaleAsync.fulfilled, (state, action: PayloadAction<null>) => {
      return initialState;
    });
    builder.addCase(updateStringPropertyAsync.fulfilled, (state, action: PayloadAction<ITaleCreate>) => {
      return action.payload;
    });
    builder.addCase(updateCoordinatesAsync.fulfilled, (state, action: PayloadAction<ITaleCreate>) => {
      return action.payload;
    });
  },
});
