  ...
  const createTale = async () => {
    ...
    taleService
      ...
      .catch(err => {
        ...
        console.error('error creating tale: ', err.response.status, errorMessage);
  ...
  const markAsRead = () => {
    ...
    taleService
      ...
      .then(res => {
        ...
        if (reads) dispatch(updateStringPropertyAsync({ property: 'taleReads', value: [...reads, { tale: tale }] }));
      })
      .catch(err => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: 'white' }}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {showReadButton && (
          <Button isLoading={isLoading} style={styles.readButton} title={t('Mark as Read')} onPress={markAsRead} />
        )}
      </ScrollView>
      ...
      {route.params.isCreation && (
        <View
          ...
          <Button isLoading={isLoading} title={t('Send')} style={{ width: '50%', opacity: 0.9 }} onPress={createTale} />
        </View>
      )}
      <Modal
        animationType="slide"
