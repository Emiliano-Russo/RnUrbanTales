...
export const NewTaleImage = () => {
  ...
  const navigation = useNavigation<StackNavigationProp<NewTaleStackParamList, 'Image'>>();
      ...
      <ScrollView contentContainerStyle={styles.scrollViewContainer}>
        {imageLinks.map((link, index) => (
          <TouchableOpacity
            ...
            style={[styles.imageWrapper, newTale.image === link && styles.selectedImageWrapper]}>
            <Image source={{ uri: link }} style={styles.image} />
          </TouchableOpacity>
        ))}
      </ScrollView>

      <Button title={t('Next')} onPress={handleNext} style={{ width: 100, marginTop: 20, marginBottom: 20 }} />
    </View>
  );
};
