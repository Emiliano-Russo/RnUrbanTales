...
export const NewTaleMark = () => {
  ...
  const navigation = useNavigation<StackNavigationProp<NewTaleStackParamList, 'Mark'>>();
          ...
          {markStandar.map(mark => {
            return (
              <View>
                <TouchableOpacity
                  onPress={() => {
                    dispatch(updateStringPropertyAsync({ property: 'mark', value: mark.name }));
                  }}
                  style={{
                    borderWidth: 1,
                    borderRadius: 60,
                    borderColor: selectedMark === mark.name ? 'black' : 'white',
                    backgroundColor: selectedMark === mark.name ? 'rgba(0, 0, 0, 0.2)' : 'trasparent',
                  }}>
                  <Image style={{ margin: 20, width: 40, height: 40 }} source={mark.image} />
                </TouchableOpacity>
              </View>
            );
          })}
              ...
              <View>
                <TouchableOpacity
                  onPress={() => {
                    ...
                    else dispatch(updateStringPropertyAsync({ property: 'mark', value: mark.name }));
                  }}
                  style={{
                    borderWidth: 1,
                    borderRadius: 60,
